...
export default async function handler(req: NextApiRequest, res: NextApiResponseServerIO) {
    ...
    if (method === 'DELETE')
        try {
            ...
            const sentEntry = currentUser.friends_added.findIndex((user: any) => JSON.stringify(user.user) === frndID)
            if (sentEntry > -1)
                currentUser.friends_added.splice(sentEntry, 1)

            const recievedEntry = potentialFrnd.friends_added.findIndex((user: any) => JSON.stringify(user.user) === frndID)
            if (recievedEntry > -1)
                potentialFrnd.friends_added.splice(recievedEntry, 1)
